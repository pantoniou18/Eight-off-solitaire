...
toFA bd
    | bd/=cafBd = toFA cafBd --difference between board and colAcesToFoundations
    | bd/=rtfBd = toFA rtfBd 
    | bd/=chfBd = toFA chfBd
    |otherwise = bd
    where
...
reserveAcesToFoundations (fnds,cols,res)
    |(null fnds) = ((filter isAce res),(filter (not.null) cols),(filter (not.isAce) res)) -- composition there
...
colAcesToFoundations (fnds,cols,res) = 
    (fnds ++ (filter isAce (filter (not.null)colHeads) ),
        (filter (not.null) (map (\col -> if (isAce (head col)) then (tail col) else col) (filter (not.null) cols))),
            res)
    where
        colHeads = map head (filter (not.null) cols)
------------------------------------------------------------
...
colHeadsToFoundations (fnds,cols,res) = 
    ((map (\f-> if (not(isKing f))&&(elem (sCard f) (filter (not.null)colHeads)) then (sCard f) else f) fnds), -- **f can't be king update foundations
